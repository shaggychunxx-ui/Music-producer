...
from dsp_kb import book, chapters, concepts, recipes
...
def search_kb(query: str, limit: int = 25, include_extract: bool = True):
    ...
    for path, text in _walk(corpus):
        ...
        if all(t in blob for t in terms):
            hits.append({"source": "kb", "path": path, "text": text[:450]})
            if len(hits) >= limit:
                return hits
    if include_extract and _EXTRACT.exists() and len(hits) < limit:
        ...
        for block in raw.split("---PAGE "):
            ...
            if all(t in low for t in terms):
                i = low.find(terms[0])
                sn = body[max(0, i - 80) : i + 320].replace("\n", " ")
